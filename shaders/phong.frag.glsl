...
void main() {
	...
	vec3 L = normalize(gl_LightSource[1].position.xyz - eyePosition);
	...
	vec4 color;
	vec3 intensity = diffuse + specular + ambient;
	
	if (Rd > 0.95){
		color = vec4(0.8,0.79,0.9,1.0);
    }
	else if (Rd > 0.5) {
		color = vec4(0.56,0.54,0.72,1.0);
	}
	else if (Rd > 0.25) {
		color = vec4(0.4,0.39,0.61,1.0);
	}
	else {
		color = vec4(0.3,0.29,0.51,1.0);
		}
		
	// This actually writes to the frame buffer
	gl_FragColor = color; //vec4(diffuse + specular + ambient, 1);
}
